...
The notice goes to the first Claude Code session up the spawner chain: a Codex thread that spawned the refused one could not act on it either. A chain that starts at a shell has no session to tell; the bridge log records the refusal, and the thread's own answer says what was refused. A thread whose escalations you intend to decide yourself should be started `--review-by-parent`, where there is no guardian, the request is answered directly, and the approval runs the command.
