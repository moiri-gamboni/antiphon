...
**What approving a refusal can and cannot do.** The reviewer re-reviews the retry, and for an action it judges high-risk it refuses again, having been told of the approval: a captured retry was denied with "the user explicitly re-approved the exact command, but it still exfiltrates a credentials file to an untrusted external destination" (`tests/fixtures/guardian-retry.jsonl`). That is Codex's guardian, not antiphon, and no message from a spawner overrides it. So approving a forwarded refusal is worth doing for an action the reviewer merely judged risky, and cannot rescue one it judged dangerous. A thread whose escalations you intend to decide yourself should be started `--review-by-parent`, where there is no guardian, the request is answered directly, and the approval runs the command.
