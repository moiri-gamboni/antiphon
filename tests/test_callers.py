...
def test_permits_read_verbs_allowed_to_everyone():
    for kind in ("claude", "codex", "human"):
        caller = Caller(
            kind=kind,
            claude_pid=None,
            claude_session_id="session-1" if kind == "claude" else None,
            codex_thread="thread-1" if kind == "codex" else None,
        )
        for verb in ("start", "resume", "ls", "ping", "status", "wait"):
            assert permits(caller, verb, "someone-else") is True
...
def test_permits_unrecognised_verb_denied_for_every_kind():
    callers = (
        Caller(kind="claude", claude_pid=1, claude_session_id="session-1", codex_thread=None),
        Caller(kind="codex", claude_pid=None, claude_session_id=None, codex_thread="thread-1"),
        Caller(kind="human", claude_pid=None, claude_session_id=None, codex_thread=None),
    )
    for caller in callers:
        assert permits(caller, "attach", None) is False
...
def test_permits_codex_send_to_unspawned_thread_allowed():
    caller = Caller(kind="codex", claude_pid=None, claude_session_id=None, codex_thread="thread-1")
    assert permits(caller, "send", "thread-2") is True
...
def test_permits_notify_allowed_only_to_codex():
    codex_caller = Caller(kind="codex", claude_pid=None, claude_session_id=None, codex_thread="thread-1")
    claude_caller = Caller(kind="claude", claude_pid=1, claude_session_id="session-1", codex_thread=None)
    human_caller = Caller(kind="human", claude_pid=None, claude_session_id=None, codex_thread=None)
    assert permits(codex_caller, "notify", None) is True
    assert permits(claude_caller, "notify", None) is False
    assert permits(human_caller, "notify", None) is False
