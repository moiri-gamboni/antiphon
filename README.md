...
- `claude` and `codex` on the bridge's PATH (`start --claude` runs `claude --bg`, and the bridge starts the Codex daemon)
...
`send` reaches the session as an ordinary cross-session message labelled `[from <thread name> via antiphon]`; the session replies with `SendMessage(to: <thread name>)` and that reply arrives in the thread as a turn. `wait` subscribes to the session's idle notice and returns its detail; a session already idle returns at once, so use it after sending something. `interrupt` has no counterpart at all — Claude Code does not offer one — so send a correction, which a busy session takes as its next message, or `antiphon stop`.
...
- A session started by a human (no `--claude`) has no spawner to ask: the call still blocks and shows in `antiphon ls`; answer it from a shell before Claude Code's 600 s hook timeout, or it is denied.
- If nothing can be asked — no bridge, a session antiphon did not start — the call is denied with the reason. A guard that fails open is not a guard.
...
- A started session's tool calls are never forwarded: `antiphon status <name>` shows no `hook` entries and `log/bridge.out` has no `Permission needed` line. `start --claude` prints a warning on stderr when it could not find `hooks/claude-permission-forward.sh`; the session then decides its own permissions.
